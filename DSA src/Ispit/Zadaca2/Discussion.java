...
class Forum {
    ...
    {
        SLLNode<Discussion> move = finance.getFirst();
        int minPopularity = 9999;
        SLLNode<Discussion> pom = null;
        while(move!=null)
        {
            int importance = 0;
            importance = move.element.getPopularity() * move.element.getUsers();
            if(importance<minPopularity)
            {
                minPopularity=importance;
                pom = move;
            }
            move=move.succ;
        }
        finance.delete(pom);
        SLLNode<Discussion> dvizi = health.getFirst();
        SLLNode<Discussion> chuvam = null;
        int max = 0;
        while(dvizi!=null)
        {
            int maxImportance = 0;
            maxImportance = dvizi.element.getPopularity() * dvizi.element.getUsers();
        ...
        finance.insertLast(chuvam.element);
    }
