...
public class Heroes
{
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards)
    {
        ...
        SLLNode<Card> herojMax = null;
        int max = 0;
        while(prv!=null)
        {
            int najaka = 0;
            najaka = prv.element.getPower()*prv.element.getNumAttacks();
            if(najaka>max)
            {
                max=najaka;
                herojMax = prv;
        ...
        secondFriendCards.insertAfter(herojMax.element,tretiot);
        firstFriendCards.delete(herojMax);
    }
