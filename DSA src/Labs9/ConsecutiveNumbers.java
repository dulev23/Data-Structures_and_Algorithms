...
class BTree<E> {
    ...
    public boolean isConsecutive() {
        return isConsecutiveRecursive(root, null);
    }

    private boolean isConsecutiveRecursive(BNode<E> node, BNode<E> prevValue) {
        if (node == null) {
            return true;
        }

        if (node.left != null) {
            if (prevValue != null && node.info.equals(prevValue.info)) {
                return false;
            }
            if (prevValue != null && node.info.equals((Integer) prevValue.info + 2)) {
                return true;
            } else {
                return isConsecutiveRecursive(node.left, node);
            }
        }
        ...
        if (node.right != null) {
            ...
            if (prevValue != null && node.info.equals((Integer) prevValue.info + 2)) {
                ...
                return isConsecutiveRecursive(node.right, prevValue);
            }
        }
        return false;
    }
...
public class ConsecutiveNumbers {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //  vasiot kod ovde
        boolean cons = tree.isConsecutive();
